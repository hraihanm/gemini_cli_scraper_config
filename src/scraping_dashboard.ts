...
async function handleBrowserFetch(body: {
  ...
  method?: string;
  body?: string | null;
}): Promise<{ html: string; status: number; url: string; error?: string }> {
  // Enqueue behind previous requests
  let resolve!: () => void;
  const thisRequest = new Promise<void>(r => { resolve = r; });
...
function loadQueue(): QueuePage[] {
  return (readJson(QUEUE_FILE, []) as QueuePage[]);
}

function loadOutputs(collection?: string): unknown[] {
  ...
  for (const f of files) {
    ...
    const data = readJson(path.join(OUTPUTS_DIR, f), []) as unknown[];
...
function buildStatus() {
  ...
  for (const col of getCollections()) {
    const data = readJson(path.join(OUTPUTS_DIR, `${col}.json`), []) as unknown[];
...
function spawnRunner(
  action: string,
  opts: { page_type?: string; count?: number },
  ...
  if (opts.page_type) rubyArgs.push('--page-type', opts.page_type);
  if (opts.count) rubyArgs.push('--count', String(opts.count));
  rubyArgs.push('--dashboard-port', String(port));

  res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });

  const child = spawn('ruby', rubyArgs, { env: process.env });
  const lines: string[] = [];
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (pathname === '/api/outputs' && method === 'GET') {
    const collection = parsedUrl.query['collection'] as string | undefined;
  ...
  if (pathname === '/api/queue' && method === 'GET') {
    let queue = loadQueue();
    const pt = parsedUrl.query['page_type'] as string | undefined;
    const st = parsedUrl.query['status'] as string | undefined;
    if (pt) queue = queue.filter(p => p.page_type === pt);
    if (st) queue = queue.filter(p => p.status === st);
  ...
  if (pathname === '/api/action' && method === 'POST') {
    ...
    let body: { action?: string; page_type?: string; count?: number } = {};
  ...
  if (pathname === '/api/browser-fetch' && method === 'POST') {
    const raw = await readBody(req);
    let body: { url?: string; headers?: Record<string, string>; method?: string; body?: string } = {};
    try { body = JSON.parse(raw); } catch { /* ignore */ }
    if (!body.url) {
