...
### Antigravity CLI — Two-Layer Model

Antigravity CLI (`agy`) uses two layers. Keep them separate:
...
**Do not:** put a `plugin.json` at `.agents/` root, add flat `.agents/skills/<name>.md` files, create a `.agents/workflows/` directory, or check in `.agents/plugins/.../skills/` (generated by the setup script, gitignored). The Gemini-era `.gemini/`, `GEMINI.md` were removed on 2026-06-18 (`gemini` binary deprecated).

**Skill frontmatter rules (CRITICAL):** The `description:` value in every `SKILL.md` **must be double-quoted** if it contains any YAML flow-indicator characters: `[`, `]`, `{`, `}`, `,`. These are forbidden in unquoted YAML scalars — AGY silently drops any skill whose frontmatter fails to parse. Usage strings with optional-arg notation (e.g. `[project=dmart-dloc]`) always trigger this. Always write:
```yaml
description: "Phase N ... [optional-arg] ..."   # quoted — safe
```
not:
```yaml
...
Each command is a skill in `.agents/skills/<name>/SKILL.md`. `project=` selects a profile from `profiles/`.
...
- Extra phases (dhero): `/restaurant-details-parser` → `/menu-listings-parser` → `/menu-parser`

**Full pipeline (one session):** `/run-pipeline project=<x> url=<u> name=<slug> [kind=html|api]` runs every phase end-to-end.
...
The agent runs `browser_grep_html(query: "@type")` first in Phase 3 to detect JSON-LD before spending time on CSS selector discovery. This is codified in the details-parser skill and its phase doc.
