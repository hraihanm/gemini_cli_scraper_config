# Greenfield Phase 2: Navigation Parser

**version:** 1.0.0

**Used by:** `profiles/greenfield.toml` (prompt-driven sources; includes search portals and registries)
**Reads:** `discovery-state.json` (use `_notes` inside JSON; also read **`crawl_model`** and **`seed_strategy`**)
**Writes:** `navigation-selectors.json` (includes human **`_notes`**)
**Edits:** parser files listed in `boilerplate.parsers` (excluding `details.rb`)
**Next phase:** `details-parser` from profile
...
## Greenfield — search portals and seed lists

- If `discovery-state.json.crawl_model` is **`search_portal`**: navigation code often belongs in **`categories.rb`** (iterate a **master list** of queries or seeds — file under `.scraper-state/` or repo path documented in `_notes`) and/or **`listings.rb`** (result rows, next-page). Blank or global search rarely yields full coverage — implement the **`seed_strategy`** from Phase 1.
- Prefer **`browser_detect_pagination`** and network capture when results load via XHR.
- Pass stable identifiers in `vars` (e.g. registration number) for downstream dedup.
- If `crawl_model` is **`flat_listings`**, you may disable or simplify `categories.rb` / `subcategories.rb` **only** after verifying `config.yaml` and DataHen page types remain consistent — document in `navigation-selectors.json._notes`.
